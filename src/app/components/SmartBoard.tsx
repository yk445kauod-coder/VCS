  ...
  return (
    <div className="bg-[#2d3748] p-3 rounded-2xl shadow-2xl border-8 border-[#1a202c] relative overflow-hidden ring-4 ring-slate-200">
      <div className="bg-[#2d3748] h-full w-full rounded-lg p-6 relative">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 h-full items-start">
            
            {/* Sticky Notes Section */}
            <div className="space-y-8">
              <div className="bg-[#fef3c7] p-6 shadow-xl -rotate-1 transform transition hover:rotate-0 hover:scale-[1.02] duration-300 relative font-handwriting">
                 ...
                 <div className="flex items-center gap-2 mb-4 border-b-2 border-yellow-500/20 pb-2">
                    ...
                    <h3 className="font-bold text-2xl text-slate-800">نقاط ذهبية</h3>
                 </div>
              ...
              {output.infographicData && output.infographicData.length > 0 && (
                <div className="grid grid-cols-1 gap-4">
                  {output.infographicData.map((fact, i) => (
                      <div key={i} className={`p-4 shadow-lg flex items-center gap-4 ${
                          i % 2 === 0 ? 'bg-teal-100/80 rotate-1' : 'bg-indigo-100/80 -rotate-1'
                      } backdrop-blur-sm border border-white/20`}>
                        <span className={`font-black text-3xl opacity-20 ${i % 2 === 0 ? 'text-teal-800' : 'text-indigo-800'}`}>#{i + 1}</span>
